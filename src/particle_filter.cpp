...
void ParticleFilter::prediction(double delta_t, double std_pos[], double velocity, double yaw_rate) {
  ...
  for(unsigned int i=0; i<num_particles; i++) {
      ...
      // bicycle motion model for when yaw rate is zero and non-zero
      //
      if(yaw_rate==0) {
...
void ParticleFilter::updateWeights(double sensor_range, double std_landmark[],
  ...
  for (unsigned int n=0; n<num_particles; n++) {
      ...
      vector <LandmarkObs> observation_vector;
      LandmarkObs obs;

      for (unsigned int i=0; i<observations.size(); i++) {
          LandmarkObs observ_inst;
          obs = observations[i];

          //perform the space transformation from vehicle to map
          observ_inst.x = particles[n].x+(obs.x*cos(particles[n].theta)-obs.y*sin(particles[n].theta));
          observ_inst.y = particles[n].y+(obs.x*sin(particles[n].theta)+obs.y*cos(particles[n].theta));
          observation_vector.push_back(observ_inst);
      }

      particles[n].weight = 1.0;

      for (unsigned int i = 0; i<observation_vector.size(); i++) {
          ...
          for (unsigned int j = 0; j < map_landmarks.landmark_list.size(); j++) {
              ...
              double calc_dist = sqrt(pow(observation_vector[i].x - landmark_x, 2.0) +
                                      pow(observation_vector[i].y - landmark_y, 2.0));
...
void ParticleFilter::resample() {
  ...
  for(unsigned int i=0; i<num_particles; i++) {
      resampled_p.push_back(particles[distribution(gen)]);
  }
